...
class Announcement
{
    public static function createAnnouncement($data)
    {
        ...
        while($row = mysqli_fetch_array($data)) {
            ...
            if ($i == 0) {
                $firstContent .= self::createFirstAnnouncementContent($row['title'], $row['notice']);
            }
            $i++;
        }
        $result .= $firstContent;
    ...
    {   $result = '';
        while ($row = mysqli_fetch_array($data)) {
            $result .= '<p class="text-center fs-5 fw-bold">'.$row['title'] .'</p>
                        <hr>
                            <div>
                                <p class="text-center">'.$row['notice'].'</p>
                            </div>';
        }
        return $result;
    }
